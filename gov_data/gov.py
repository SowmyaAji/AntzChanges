...
df = df.set_index(['country_name', 'country_code',
...
def get_values(inlist=list(d)):
    ...
    country_val = {}
    for row in inlist:
        country_val.setdefault(
            (row['country_name'], row['country_code'],
             row['year']), []).append(row['att'])
    return country_val
...
def column_names():
    ...
    """
    return ['country_name', 'country_code', 'year',
            'cc_per_rnk', 'ge_per_rnk', 'rq_per_rnk',
            'rl_per_rnk', 'va_per_rnk']
